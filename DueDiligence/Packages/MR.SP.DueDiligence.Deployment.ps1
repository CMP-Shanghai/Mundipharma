...
$Host.UI.RawUI.WindowTitle = "-- Mundipharma Diligence Portal Deployment Script --"
...
Write-Host -ForegroundColor White "| Automated Mundipharma Diligence Portal Deployment Script|"
...
[string]$ConfigFile = $("$dp0\MR.SP.DueDiligence.Deployment.xml")
...
{	
	...
	if ($FeatureActive -eq $null)
	{
		...
		Enable-SPFeature -identity $FeatureId -URL $FeatureEffectiveUrl -force -Confirm:1
...
$postDeploymentActivatedFeatures = $xmldoc.selectnodes($xpath) | sort -descending {[int]$_.FeatureSequence}
...
$postDeploymentActivatedFeatures = $xmldoc.selectnodes($xpath) | sort  {[int]$_.FeatureSequence}
...
$EndDate = Get-Date
Write-Host -ForegroundColor White "------------------------------------"
Write-Host -ForegroundColor White "| Automated Mundipharma Diligence Portal Deploymentscript|"
Write-Host -ForegroundColor White "| Started on: $StartDate |"
